...
const active = (state = 'revision1', number = 1) => `<div class="masteryagent-agent" data-message-id="${number}">Question ${number}</div>
    ${actionForm('reply', state, '<label for="reply">Your reply</label><textarea id="reply" name="reply" required maxlength="8000"></textarea>')}
    ${actionForm('finish', state)}`;
...
test('a pending reply blocks duplicate submissions and finish', async t => {
    ...
    await page.evaluate(() => {
        ...
        document.querySelector('form[data-action="finish"]').dispatchEvent(new Event('submit', {bubbles: true, cancelable: true}));
    });
    ...
    assert.equal(await page.isDisabled('form[data-action="finish"] button'), false);
});
...
test('finish shows scores and retry starts an attempt without navigation', async t => {
    ...
    let navigations = 0;
    page.on('framenavigated', () => navigations++);
    await page.click('form[data-action="finish"] button');
